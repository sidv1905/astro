...
import { getFileInfo, safelyGetAstroExports } from '../vite-plugin-utils/index.js';
	...
	return {
		...
		async load(id, opts) {
			...
			if (id.endsWith('.md')) {
				...
				let { code: astroResult, vfile, metadata } = renderResult;
...
${isAstroFlavoredMd ? setup : ''}`.trim();
				...
				// Join file exports generated by remark and rehype plugins
				const fileExports = safelyGetAstroExports(vfile.data);
				const exportString = Object.entries(fileExports)
